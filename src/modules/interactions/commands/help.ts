...
export const raw = {
    ...
    options: [
        {
            name: "Command",
            description: "The command you want specific information about",
            type: 3,
            choices: [
                {
                   name: 'wiki',
...
export const command = {
    ...
    execute: async (interaction: interaction): Promise<void> => {
        ...
        const returned = await methods.reply(interaction, 'LOADING, PLEASE WAIT ...')
        ...
        if (interaction.data && interaction.data.options) {
            ...
            await methods.embed.defaultEmbed(returned.data.token, {
                title: 'Help',
                desc,
            })
        ...
        await methods.deleteOriginal(returned.data.token)
        await methods.embed.defaultEmbed(returned.data.token, { title: 'Help', desc: `
